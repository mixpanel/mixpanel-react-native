...
describe("Feature Flags", () => {
  ...
  describe("Native Mode - Asynchronous Methods", () => {
    ...
    describe("updateContext", () => {
      it("should update context in native mode", async () => {
        const context = {
          platform: "mobile",
          custom_properties: {
            user_type: "premium",
        ...
        mockNativeModule.updateFlagsContext.mockResolvedValue(true);

        await mixpanel.flags.updateContext(context);
  ...
  describe("JavaScript Mode", () => {
    beforeEach(async () => {
      // Mock native module as unavailable for JS mode
      const originalModule = NativeModules.MixpanelReactNative;
      NativeModules.MixpanelReactNative = {
        ...originalModule,
        loadFlags: undefined,
        areFlagsReadySync: undefined,
      };

      mixpanel = new Mixpanel(testToken, false, false, AsyncStorage);
      await mixpanel.init();

      // Mock successful decide response
    ...
    describe("loadFlags", () => {
      it("should fetch flags from decide endpoint", async () => {
        await mixpanel.flags.loadFlags();
      ...
      it("should cache flags in AsyncStorage", async () => {
        await mixpanel.flags.loadFlags();

        const cachedFlags = await AsyncStorage.getItem(
          `MIXPANEL_${testToken}_FLAGS_CACHE`
        );
        expect(cachedFlags).toBeTruthy();
      });

      it("should set flagsReady to true after loading", async () => {
      ...
      it("should handle network errors gracefully", async () => {
        ...
        await expect(mixpanel.flags.loadFlags()).resolves.not.toThrow();
      });
    ...
    describe("Synchronous Methods", () => {
      ...
      it("should get variant sync", () => {
        const variant = mixpanel.flags.getVariantSync("test-flag", {
          key: "fallback",
          value: "default",
        });
      ...
      it("should get variant value sync", () => {
        const value = mixpanel.flags.getVariantValueSync("test-flag", "default");

        expect(value).toBe("blue");
      });

      it("should check if enabled sync", () => {
        const enabled = mixpanel.flags.isEnabledSync("enabled-flag", false);

        expect(enabled).toBe(true);
      });

      it("should return fallback for unknown flag", () => {
        const variant = mixpanel.flags.getVariantSync("unknown-flag", {
          key: "fallback",
          value: "default",
        });

        expect(variant).toEqual({ key: "fallback", value: "default" });
      });

      it("should handle boolean flag values in isEnabledSync", () => {
        const enabled = mixpanel.flags.isEnabledSync("enabled-flag", false);

        expect(enabled).toBe(true);
      });

      it("should handle string '1' as enabled", () => {
        global.fetch.mockResolvedValue({
          ok: true,
          json: () =>
            Promise.resolve({
        ...
        // Reload flags with new data
        mixpanel.flags.loadFlags().then(() => {
    ...
    describe("Experiment Tracking", () => {
      ...
      it("should track $experiment_started on first flag access", async () => {
        const trackSpy = jest.spyOn(mixpanel, "track");

        await mixpanel.flags.getVariant("test-flag", {
          key: "fallback",
          value: "default",
        });
      ...
      it("should not track experiment twice for same flag", async () => {
        const trackSpy = jest.spyOn(mixpanel, "track");
      ...
      it("should not track when experiment is not active", async () => {
        global.fetch.mockResolvedValue({
          ok: true,
          json: () =>
            Promise.resolve({
        ...
        await mixpanel.flags.getVariant("inactive-flag", {
          key: "fallback",
          value: "default",
        });
      ...
      it("should not track when user is QA tester", async () => {
        global.fetch.mockResolvedValue({
          ok: true,
          json: () =>
            Promise.resolve({
        ...
        await mixpanel.flags.getVariant("qa-flag", {
          key: "fallback",
          value: "default",
        });
    ...
    describe("Context Updates", () => {
      it("should update context and reload flags", async () => {
        const context = {
          platform: "web",
          custom_properties: {
            user_type: "free",
        ...
        // Should make new request with updated context
        expect(global.fetch).toHaveBeenCalledWith(
          expect.any(String),
          expect.objectContaining({
            body: expect.stringContaining("user_type"),
          })
        );
      });
    });
