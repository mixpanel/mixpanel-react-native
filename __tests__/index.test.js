...
test(`it calls MixpanelReactNative initialize`, async () => {
  ...
  expect(NativeModules.MixpanelReactNative.initialize).toBeCalledWith("token", true, false, {"$lib_version": "2.2.2", "mp_lib": "react-native"});
});
...
test(`it calls MixpanelReactNative initialize with optOut and superProperties`, async () => {
  ...
  expect(NativeModules.MixpanelReactNative.initialize).toBeCalledWith("token", true, true, {"$lib_version": "2.2.2", "mp_lib": "react-native", "super": "property"}, "https://api.mixpanel.com");
});
