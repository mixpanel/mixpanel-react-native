...
open class MixpanelReactNative: NSObject {
    ...
    @objc
    func trackClick(_ token: String, clickEvent: [String: Any],
                    properties: [String: Any]? = nil,
                    resolver resolve: RCTPromiseResolveBlock,
                    rejecter reject: RCTPromiseRejectBlock) -> Void {
        let instance = MixpanelReactNative.getMixpanelInstance(token)
        let event = buildClickEvent(from: clickEvent)
        let mpProperties = MixpanelTypeHandler.processProperties(properties: properties)
        instance?.autocapture.trackClick(event, properties: mpProperties)
        resolve(nil)
    }

    @objc
    func trackRageClick(_ token: String, clickEvent: [String: Any],
                        properties: [String: Any]? = nil,
                        resolver resolve: RCTPromiseResolveBlock,
                        rejecter reject: RCTPromiseRejectBlock) -> Void {
        let instance = MixpanelReactNative.getMixpanelInstance(token)
        let event = buildClickEvent(from: clickEvent)
        let mpProperties = MixpanelTypeHandler.processProperties(properties: properties)
        instance?.autocapture.trackRageClick(event, properties: mpProperties)
        resolve(nil)
    }

    @objc
    func trackDeadClick(_ token: String, clickEvent: [String: Any],
                        properties: [String: Any]? = nil,
                        resolver resolve: RCTPromiseResolveBlock,
                        rejecter reject: RCTPromiseRejectBlock) -> Void {
        let instance = MixpanelReactNative.getMixpanelInstance(token)
        let event = buildClickEvent(from: clickEvent)
        let mpProperties = MixpanelTypeHandler.processProperties(properties: properties)
        instance?.autocapture.trackDeadClick(event, properties: mpProperties)
        resolve(nil)
    }

    private func buildClickEvent(from map: [String: Any]) -> ClickEvent {
        let x = map["x"] as? CGFloat ?? 0
        let y = map["y"] as? CGFloat ?? 0
        let elementId = map["elementId"] as? String ?? ""
        return ClickEvent(
            x: x,
            y: y,
            elementId: elementId,
            tagName: map["tagName"] as? String,
            accessibleLabel: map["accessibleLabel"] as? String,
