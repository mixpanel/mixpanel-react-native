...
const RFC3339_REGEX = /^(\d{4})-(\d{2})-(\d{2})[Tt](\d{2}):\d{2}:\d{2}(\.\d+)?([Zz]|[+-]\d{2}:\d{2})$/;
...
// json-logic-js dispatches custom operations variadically via `.apply`, so this reads `arguments`
// and must stay a function declaration rather than an arrow function.
function semverCompare() {
    const ops = operands(arguments);
    if (!ops) {
        return false;
    }
    ...
    const actual = normalizeSemver(ops.actual);
    const target = normalizeSemver(ops.target);
    if (!SEMVER_REGEX.test(actual) || !SEMVER_REGEX.test(target)) {
        return false;
    }
    const cmp = compareSemver(actual, target);
...
function datetimeCompare() {
    ...
    const actualSec = convertRfc3339ToUnixSeconds(ops.actual);
    const targetSec = convertUnixMillisecondsToSeconds(ops.target);
...
function normalizeSemver(version) {
    ...
    for (let i = 0; i < separators.length; i++) {
        const index = stripped.indexOf(separators[i]);
...
function convertRfc3339ToUnixSeconds(v) {
    ...
    const fields = RFC3339_REGEX.exec(normalized);
    if (!fields) {
        return null;
    }
