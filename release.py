...
def bump_version():
    ...
    subprocess.call('cd MixpanelDemo;yarn upgrade mixpanel-react-native --latest', shell=True)
    subprocess.call('git add package.json', shell=True)
    subprocess.call('git add __tests__/index.test.js', shell=True)
    subprocess.call('git add MixpanelDemo/yarn.lock', shell=True)
...
def publish():
    subprocess.call('npm publish', shell=True)
